...
const SNOOZE_MS = 10 * 60 * 1000; // report d'un rappel (« Plus tard » / réunion)
const calcLevel = points => Math.floor((points||0)/100)+1; // 1 niveau / 100 pts
const DEMO_MODE = true; // outils de démo (reset, simuler rappel, seed) ; passer à false pour la prod V2
...
const defaultState = () => ({
  ...
  // Boucle d'engagement (LOCAL uniquement — voir unlockCosmetics()).
  // unlockedCosmetics ex. ["color_yeux"] ; selectedColor "default"|"color_*" ;
  // dailyQuestClaimed ex. {"2026-06-25":true}.
  unlockedCosmetics:[], selectedColor:"default", dailyQuestClaimed:{},
});
function loadState(){
  try{
    ...
    if(s.lastActiveDate && s.lastActiveDate!==t){
      ...
      const completedYest=(s.pausesByDay[s.lastActiveDate]||0)>=(s.goalPerDay||6);
...
// hex "#RRGGBB" -> "#RRGGBB" éclairci de `amt` (0..1) vers le blanc (pour le halo).
...
function Mascot({ mood="neutral", size=140, stage=1, float=true, tint=null }){
  const uid = (React.useId ? React.useId() : "m").replace(/[:]/g,"");
  const base = STAGE_STYLE[stage] || STAGE_STYLE[1];
  // tint (couleur de zone choisie) teinte franchement le corps + l'accent/halo/contour.
  const S = tint ? {...base, from:lighten(tint,0.72), to:lighten(tint,0.30), stroke:tint, accent:tint, halo:lighten(tint,0.55), ring:tint} : base;
...
function EyeRule(){
  const uid = (React.useId ? React.useId() : "er").replace(/[:]/g,"");
  return (
    <svg viewBox="0 0 280 110" className="w-full h-auto">
      ...
      <defs><marker id={"arr"+uid} markerWidth="8" markerHeight="8" refX="6" refY="4" orient="auto">
        <path d="M0,0 L8,4 L0,8 Z" fill="#34D399"/>
      </marker></defs>
      <path d="M135,60 L230,60" stroke="#34D399" strokeWidth="2" strokeDasharray="4 3" markerEnd={"url(#arr"+uid+")"}/>
...
// ============================================================
// Boucle d'engagement (LOCAL uniquement) — cosmétiques de l'Esprit.
// Déblocages définitifs, recalculés depuis exercisesDoneIds + points via
// unlockCosmetics() (idempotent). En V1, tout reste en localStorage.
// ============================================================
// 6 zones → 1 couleur d'Esprit nommée + 1 forme distincte (jamais la couleur seule).
const ZONE_META = {
  "Nuque":    {colorId:"color_nuque",    colorName:"Indigo", hex:"#6366F1", shape:"diamond"},
  "Épaules":  {colorId:"color_epaules",  colorName:"Corail", hex:"#FB7185", shape:"triangle"},
  "Poignets": {colorId:"color_poignets", colorName:"Ambre",  hex:"#F59E0B", shape:"square"},
  "Yeux":     {colorId:"color_yeux",     colorName:"Azur",   hex:"#38BDF8", shape:"circle"},
  "Jambes":   {colorId:"color_jambes",   colorName:"Menthe", hex:"#34D399", shape:"hexagon"},
  "Dos":      {colorId:"color_dos",      colorName:"Prune",  hex:"#A855F7", shape:"pentagon"},
};
const ZONE_ORDER = ["Nuque","Épaules","Poignets","Yeux","Jambes","Dos"];
// Couleurs sélectionnables : "Esprit" par défaut (suit le stade) + 1 par zone.
const COLOR_DEFS = [
  {id:"default", name:"Esprit", hex:null, unlock:null}, // hex null → couleur du stade
  ...ZONE_ORDER.map(z=>({ id:ZONE_META[z].colorId, name:ZONE_META[z].colorName, hex:ZONE_META[z].hex, unlock:"Complète la zone "+z })),
];
const DAILY_QUEST_TARGET = 3; // 3 pauses → quête accomplie (indépendant de l'objectif de pauses/j)

const colorById = id => COLOR_DEFS.find(c=>c.id===id) || COLOR_DEFS[0];
// Couleur effective pour la mascotte : null si "default" (la mascotte garde la teinte du stade).
const colorFor = id => colorById(id).hex;

// Zone "complétée" = tous ses exercices figurent dans exercisesDoneIds.
function zoneProgress(zone, doneIds){
  const all = EXERCISES.filter(e=>e.zone===zone);
  const done = all.filter(e=>(doneIds||[]).includes(e.id));
  return { done: done.length, total: all.length, complete: all.length>0 && done.length===all.length };
}

// Recalcule l'ensemble des déblocages (zone) à partir des données existantes —
// idempotent. Appelé à chaque patch pertinent (points/exercices) pour rester stable.
function unlockCosmetics(state){
  const owned = new Set(state.unlockedCosmetics||[]);
  for(const z of ZONE_ORDER){ if(zoneProgress(z, state.exercisesDoneIds).complete) owned.add(ZONE_META[z].colorId); }
  const next = Array.from(owned);
  // Évite un nouvel objet si rien n'a changé (stabilité du state).
  const prev = state.unlockedCosmetics||[];
  if(next.length===prev.length && next.every(id=>prev.includes(id))) return state;
  return {...state, unlockedCosmetics: next};
}
...
function Lightbox({ src, alt, open, onClose }){
  ...
  if(!open) return null;
  return (
    <div role="dialog" aria-modal="true" aria-label={alt||"Image agrandie"}
      onClick={onClose}
      className="fixed inset-0 z-50 flex items-center justify-center p-4"
      style={{background:"rgba(0,0,0,.7)"}}>
      <button onClick={onClose} aria-label="Fermer"
        className="absolute top-3 right-3 w-11 h-11 grid place-items-center rounded-full bg-white/90 text-gray-800 text-2xl leading-none shadow-soft hover:bg-white">×</button>
      <img src={src} alt={alt||""} onClick={e=>e.stopPropagation()}
        style={{objectFit:"contain", maxWidth:"min(95vw,1100px)", maxHeight:"90vh"}}
        className="rounded-xl shadow-soft"/>
    </div>
  );
}

function SanteCard({ img, alt, onOpen }){
  const ref = React.useRef(null);
  return (
    <button ref={ref} onClick={()=>onOpen(ref.current)} title="Voir en grand"
      className="block w-full bg-white rounded-xl border border-gray-100 overflow-hidden shadow-soft hover:border-violet-200 hover:shadow-md transition">
      <img src={img} alt={alt} loading="lazy" decoding="async"
        style={{objectFit:"contain"}} className="w-full"/>
    </button>
  );
}
...
function Sidebar({ onClose, onCollapse }){
  ...
  const POSTURE = [
    { img:"assets/sante/mauvaise_posture.png",
      alt:"Mauvaise posture à éviter : dos voûté, tête trop penchée vers l'écran et épaules tendues" },
    { img:"assets/sante/bonne_posture.png",
      alt:"Bonne posture à adopter : dos droit, écran à hauteur des yeux et épaules détendues" },
  ];
  const REGLE = [
    { img:"assets/sante/regle20-20-20_1.png",
      alt:"Règle 20-20-20, étape 1 : toutes les 20 minutes" },
    { img:"assets/sante/regle20-20-20_2.png",
      alt:"Règle 20-20-20, étape 2 : regarder un objet situé à environ 6 mètres" },
    { img:"assets/sante/regle20-20-20_3.png",
      alt:"Règle 20-20-20, étape 3 : pendant 20 secondes" },
  ];
  const openZoom = (s, el) => { triggerRef.current = el; setZoom(s); };
  const closeZoom = () => { setZoom(null); const el = triggerRef.current; if(el) el.focus(); triggerRef.current = null; };
  return (
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="flex-1 overflow-y-auto scroll-pretty">
        ...
        <div className="px-5 py-4 space-y-5">
          <section className="space-y-2">
            <h3 className="font-bold text-sm">Bien s'installer</h3>
            <div className="space-y-3">
              {POSTURE.map(s=>(
                <SanteCard key={s.img} img={s.img} alt={s.alt} onOpen={el=>openZoom(s, el)}/>
              ))}
            </div>
          </section>
      ...
      <Lightbox open={!!zoom} src={zoom?.img} alt={zoom?.alt} onClose={closeZoom}/>
    </div>
  );
}
        ...
        {NAV_ITEMS.map(it=>{
          ...
          return (
            <li key={it.id} className="relative">
              <button onClick={()=>onNavigate(it.id)} className="w-full py-3 flex flex-col items-center gap-0.5">
      ...
      {NAV_ITEMS.map(it=>{
        ...
        return (
          <button key={it.id} onClick={()=>onNavigate(it.id)} title={it.label}
            className={"relative w-[72px] py-2.5 rounded-2xl flex flex-col items-center gap-1 transition hover:bg-violet-50"+(isActive?" bg-violet-100":"")}>
            <Icon name={it.icon} size={26} className={isActive?"pop":""}/>
    ...
    <div>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-soft mb-3">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <select aria-label="Heure de début" value={state.activeStartHour} onChange={e=>onPatch({activeStartHour:parseInt(e.target.value,10)})} className="border border-gray-200 rounded-lg px-2 py-1.5 bg-white">{hours.map(h=><option key={h} value={h}>{h}h</option>)}</select>
          <span className="text-gray-500">à</span>
          <select aria-label="Heure de fin" value={state.activeEndHour} onChange={e=>onPatch({activeEndHour:parseInt(e.target.value,10)})} className="border border-gray-200 rounded-lg px-2 py-1.5 bg-white">{hours.map(h=><option key={h} value={h}>{h}h</option>)}</select>
        </div>
    ...
    <div className="min-h-screen relative z-10 flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-white rounded-3xl p-6 shadow-soft text-center">
        ...
        <div className="text-left mt-5">
          <label htmlFor="welcome-name" className="text-xs font-semibold text-gray-600">Ton prénom</label>
          <input id="welcome-name" value={name} onChange={e=>setName(e.target.value)} placeholder="Ex : Marie" className="w-full mt-1 border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:border-violet-400"/>
        </div>
...
// Forme distincte par zone (jamais la couleur seule → accessibilité).
function ZoneShape({ shape, color, size=22 }){
  const c=size/2, common={fill:color, stroke:"#fff", strokeWidth:1.5};
  const polys = {
    triangle: c+","+(size*0.12)+" "+(size*0.9)+","+(size*0.85)+" "+(size*0.1)+","+(size*0.85),
    diamond:  c+","+(size*0.08)+" "+(size*0.92)+","+c+" "+c+","+(size*0.92)+" "+(size*0.08)+","+c,
    pentagon: c+","+(size*0.08)+" "+(size*0.95)+","+(size*0.42)+" "+(size*0.78)+","+(size*0.95)+" "+(size*0.22)+","+(size*0.95)+" "+(size*0.05)+","+(size*0.42),
    hexagon:  (size*0.25)+","+(size*0.1)+" "+(size*0.75)+","+(size*0.1)+" "+(size*0.95)+","+c+" "+(size*0.75)+","+(size*0.9)+" "+(size*0.25)+","+(size*0.9)+" "+(size*0.05)+","+c,
  };
  return (
    <svg width={size} height={size} viewBox={"0 0 "+size+" "+size} aria-hidden="true">
      {shape==="circle"   && <circle cx={c} cy={c} r={c*0.82} {...common}/>}
      {shape==="square"   && <rect x={size*0.12} y={size*0.12} width={size*0.76} height={size*0.76} rx={size*0.14} {...common}/>}
      {polys[shape] && <polygon points={polys[shape]} {...common}/>}
    </svg>
  );
}

// Collection par zone. Chaque zone complétée débloque sa couleur d'Esprit,
// applicable direct en tapant la carte (re-tap = retour couleur par défaut).
// Zone non complétée : tap → bibliothèque pour faire ses exercices.
function ZoneCollection({ doneIds, selectedColor, onApplyColor, onOpenZone }){
  return (
    <div className="bg-white rounded-2xl p-4 shadow-soft mb-4">
      <div className="text-sm font-semibold">Collection par zone</div>
      <div className="text-[11px] text-gray-500 mb-3">Complète une zone pour débloquer sa couleur d'Esprit — puis applique-la d'un tap.</div>
      <div className="grid grid-cols-2 gap-2.5">
        {ZONE_ORDER.map(z=>{
          const m=ZONE_META[z], p=zoneProgress(z, doneIds);
          const active = selectedColor===m.colorId;
          return (
            <button key={z} onClick={p.complete ? ()=>onApplyColor(m.colorId) : ()=>onOpenZone(z)}
              title={p.complete?("Appliquer la couleur "+m.colorName):"Voir les exercices de la zone"}
              className={"btn-press text-left rounded-2xl p-3 border min-h-[44px] transition "+(active?"bg-violet-50 border-violet-300 ring-1 ring-violet-200":p.complete?"bg-emerald-50 border-emerald-200":"bg-white border-gray-100 hover:border-violet-200")}>
              <div className="flex items-center gap-2">
                <ZoneShape shape={m.shape} color={m.hex} size={22}/>
                <span className="text-sm font-semibold leading-tight flex-1 min-w-0 truncate">{z}</span>
                {p.complete && <span className="text-emerald-500 text-sm" title="Zone complétée">✓</span>}
              </div>
              <div className="text-[11px] text-gray-500 mt-1.5 tabular-nums">{p.done}/{p.total} exercices</div>
              <div className="text-[10px] mt-0.5 leading-tight font-medium" style={{color:m.hex}}>
                {!p.complete ? ("Couleur "+m.colorName+" à débloquer") : active ? ("● Couleur "+m.colorName+" active") : ("Appliquer la couleur "+m.colorName)}
              </div>
            </button>
          );
        })}
...
function Profile({ state, onPatch, onOpenZone, onApplyColor }){
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-3xl p-5 shadow-soft mb-4 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <input aria-label="Ton prénom" value={profile.name} onChange={e=>onPatch({name:e.target.value, guest:false})} className="w-full text-lg font-bold border-b border-transparent hover:border-gray-200 focus:border-violet-400 focus:outline-none"/>
      ...
      <ZoneCollection doneIds={state.exercisesDoneIds} selectedColor={state.selectedColor} onApplyColor={onApplyColor} onOpenZone={onOpenZone}/>
...
// ============================================================
function Team(){
  const scores = loadTeamScores(); // V2 : loadTeamScores(state.teamCode)
  return (
    <div>
      <div className="mb-3">
...
// ============================================================
// Carte « Quête du jour » (Accueil). Progression vers DAILY_QUEST_TARGET pauses.
// À l'objectif : « Quête accomplie » + bouton claim (1 claim/jour, +15 pts).
// Au-delà : message doux, jamais « terminé/limite ». Ton bienveillant.
// ============================================================
function DailyQuestCard({ pausesToday, claimed, onClaim }){
  const target = DAILY_QUEST_TARGET;
  const reached = pausesToday >= target;
  const shown = Math.min(pausesToday, target);
  return (
    <section className="bg-white rounded-3xl p-5 shadow-soft mb-4">
      <div className="flex items-center justify-between mb-2">
        <div className="text-xs uppercase tracking-wide text-violet-500 font-semibold">🎯 Quête du jour</div>
        <div className="text-xs text-gray-500 tabular-nums">{shown}/{target}</div>
      </div>
      {!reached && (
        <p className="text-sm text-gray-600 leading-snug">Fais <b>{target} pauses</b> aujourd'hui pour gagner la récompense du jour.</p>
      )}
      {/* Jalons : ● faits / ○ à venir. Forme + nombre, jamais la couleur seule. */}
      <div className="flex items-center gap-2 mt-3" aria-label={shown+" pauses sur "+target}>
        {Array.from({length:target}).map((_,i)=>(
          <span key={i} className={"w-3.5 h-3.5 rounded-full "+(i<pausesToday?"bg-violet-500":"bg-gray-200")}/>
        ))}
        <span className="text-xs text-gray-400 ml-1 tabular-nums">{shown}/{target}</span>
      </div>
      {reached && !claimed && (
        <div className="mt-4">
          <div className="text-sm font-bold text-emerald-600 mb-2">✓ Quête accomplie !</div>
          <button onClick={onClaim} className="btn-press w-full py-3 rounded-2xl text-white font-semibold shadow-soft" style={{background:"linear-gradient(135deg,#34D399,#10B981)"}}>🎁 Récupérer ma récompense</button>
        </div>
      )}
      {reached && claimed && (
        <div className="mt-4 flex items-center gap-2 text-sm text-emerald-700 bg-emerald-50 rounded-2xl px-4 py-3">
          <span className="text-lg">✓</span>
          <span className="font-medium leading-snug">Récompense récupérée ! Bravo, l'Esprit pétille ✨</span>
        </div>
      )}
    </section>
  );
...
function App(){
  ...
  // Recalcule les déblocages de couleurs (zones complétées) à chaque changement pertinent.
  useEffect(()=>{ setState(s=>unlockCosmetics(s)); },[state.points, state.exercisesDoneIds]);
  ...
  function takeBreakNow(){
    ...
    if(pausesToday+1===state.goalPerDay) next.streak+=1;
  ...
  function snooze(){
    ...
    next.nextReminderAt=Date.now()+SNOOZE_MS;
    setState(next);
  }
  function resetDemo(){
  ...
  function fireReminder(){
    ...
    if(state.calendarMockEnabled && state.inMeeting){
      const next={...state,nextReminderAt:Date.now()+SNOOZE_MS};
      setState(next);
  ...
  // Claim de la Quête du jour : 1 claim/jour, récompense = +15 points, plafond gardé.
  function claimDailyQuest(){
    setState(s=>{
      if((s.pausesByDay[t]||0) < DAILY_QUEST_TARGET || s.dailyQuestClaimed[t]) return s; // garde-fou plafond
      return {...s, dailyQuestClaimed:{...s.dailyQuestClaimed, [t]:true}, points:(s.points||0)+15};
    });
    setJustAwarded(true); setTimeout(()=>setJustAwarded(false),900);
    showNotice("🎉 Quête du jour accomplie ! +15 points");
  }
  // Applique une couleur débloquée à l'Esprit (re-tap = retour à la couleur par défaut du stade).
  function applyColor(id){ setState(s=>({...s, selectedColor: s.selectedColor===id ? "default" : id})); }
  ...
  return (
    <div className={"min-h-screen relative lg:pl-[88px] transition-[padding] duration-300 "+(santeOpen?"lg:pr-[380px] xl:pr-[420px]":"lg:pr-0")}>
      ...
      <main className="relative max-w-2xl mx-auto px-4 pt-6 pb-28 lg:pb-10">
        <header className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-[11px] text-gray-500 capitalize">{new Date().toLocaleDateString("fr-FR",{weekday:"long",day:"numeric",month:"long"})}</div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            {DEMO_MODE && (<button onClick={resetDemo} title="Réinitialiser (démo)" className="text-xs px-3 py-1.5 rounded-full bg-white border border-gray-200 text-gray-500 hover:bg-gray-50">↻ démo</button>)}
          </div>
        </header>

        {view==="home" && (<>
        <DailyQuestCard pausesToday={pausesToday} claimed={!!state.dailyQuestClaimed[t]} onClaim={claimDailyQuest}/>
        <section className="bg-white rounded-3xl p-5 shadow-soft flex items-center gap-4 mb-4">
          <Mascot mood={mood} size={120} stage={mascotStage} tint={colorFor(state.selectedColor)}/>
        ...
        {DEMO_MODE && (<section className="rounded-3xl p-4 mb-2 border border-dashed border-gray-300 bg-white/70">
        ...
        {view==="profile" && (<Profile state={state} onPatch={patchProfile} onOpenZone={()=>navigate("library")} onApplyColor={applyColor}/>)}
